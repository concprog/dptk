...
def detect_aruco(dict_type: int = cv2.aruco.DICT_6X6_250, metadata_key: str = "aruco"):
    """
...
def estimate_pose_single_markers(
    ...
    def wrapper(ctx: FrameContext) -> FrameContext:
        ...
        ctx.metadata[metadata_output_key] = {"rvecs": rvecs, "tvecs": tvecs}
        return ctx
...
def draw_axis(
    ...
    dist_coeffs: np.ndarray,
    length: float = 0.03,
    metadata_pose_key: str = "pose",
    metadata_aruco_key: str = "aruco",
):
    """
    ...
    def wrapper(ctx: FrameContext) -> FrameContext:
        ...
        for i, (rvec, tvec) in enumerate(zip(rvecs, tvecs)):
            if rvec is not None and tvec is not None:
                cv2.drawFrameAxes(
                    ctx.frame, camera_matrix, dist_coeffs, rvec, tvec, length
                )
